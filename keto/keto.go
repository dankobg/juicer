...
func NewClient() (*Client, error) {
	rConn, err := grpc.NewClient("keto:4466", grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		return nil, fmt.Errorf("failed to connect to keto grpc read server: %w", err)
	}

	wConn, err := grpc.NewClient("keto:4467", grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
