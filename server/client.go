...
func getMsgType(typ int) string {
	switch typ {
	case 1:
		return "text"
	case 2:
...
// reader reads messages from the websocket connection and passes to hub for processing.
...
func (c *client) reader() {
	...
	for {
		...
		c.log.Debug("reader msg", slog.String("client_id", c.id), slog.String("msg_type", msgType), slog.String("msg", msg.String()))

		if err := c.hub.processClientMessage(c, msg); err != nil {
			c.log.Error("hub process client message", slog.Any("client_id", c.id), slog.Any("error", err))
			c.sendErrorMsg(err)
			continue
		}
	}
