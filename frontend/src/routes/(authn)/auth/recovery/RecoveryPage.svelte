<script lang="ts">
  ...
  const { form, enhance, errors } = superForm(initialRecoveryForm, {
    validators: recoveryFormSchema,
    SPA: true,
    dataType: 'json',
    ...
    async onUpdated({ form }) {
      ...
      const body = form.data as UpdateRecoveryFlowWithCodeMethod;
    ...
    <div class="form-wrapper">
      ...
      {#each data?.flow?.ui?.messages ?? [] as msg}
        <ToastNotification
          ...
          subtitle={msg.type === 'error' ? 'Unable to recover account' : undefined}
          caption={msg.text}
          lowContrast
          fullWidth
        />
