<script lang="ts">
	...
	import { focusedIndex, otpArray } from './store';
	import { REGEXP_ONLY_CHARS, REGEXP_ONLY_DIGITS, REGEXP_ONLY_DIGITS_AND_CHARS, type rootProps } from './types.js';
	...
	export let formatPasteData: (text: string) => string = (text: string) => text;
	...
	function handlePaste(event: ClipboardEvent) {
		...
		const formattedPasteData = formatPasteData(pasteData);
		const otp = formattedPasteData.slice(0, maxLength);
		const regex = getPattern();
		if (regex.test(otp)) {
			updateOtpArray(otp);
		}
