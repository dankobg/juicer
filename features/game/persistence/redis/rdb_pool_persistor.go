...
var _ game.PoolPersistor = (*RedisPoolPersistor)(nil)
...
func NewRedisPoolPersistor(rdb *redis.Client) *RedisPoolPersistor {
...
func (pst *RedisPoolPersistor) MatchPair(ctx context.Context, pool game.Pool) ([2]string, error) {
...
func (pst *RedisPoolPersistor) JoinPool(ctx context.Context, userID uuid.UUID, pool game.Pool) error {
	var (
		poolKey         = fmt.Sprintf("pool:%s", pool.Name())
...
func (pst *RedisPoolPersistor) ListPoolPlayers(ctx context.Context, pool game.Pool) ([]string, error) {
	poolKey := fmt.Sprintf("pool:%s", pool.Name())
