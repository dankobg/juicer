...
var _ presence.PresencePersistor = (*RedisPresencePersistor)(nil)
...
func NewRedisPresencePersistor(rdb *redis.Client) *RedisPresencePersistor {
...
func (pst *RedisPresencePersistor) SetPresence(ctx context.Context, userID uuid.UUID, connID uuid.UUID, username string, guest bool, channels []string) (presence.PresenceChannelsDiff, error) {
	now := time.Now()
	...
	if err != nil {
		return presence.PresenceChannelsDiff{}, err
	}

	arr, ok := result.([]any)
	if !ok {
		return presence.PresenceChannelsDiff{}, fmt.Errorf("invalid lua result: %T", result)
	}

	if len(arr) != 2 {
		return presence.PresenceChannelsDiff{}, fmt.Errorf("invalid lua result length: expected 2, got: %d", len(arr))
	}
	...
	if !ok1 || !ok2 {
		return presence.PresenceChannelsDiff{}, fmt.Errorf("invalid result sub arrays")
	}

	presenceDiffInfo := presence.PresenceChannelsDiff{
	...
	for i, item := range connJoinedArr {
		v, ok := item.(string)
		if !ok {
			return presence.PresenceChannelsDiff{}, fmt.Errorf("invalid result sub arrays not string")
		}
...
func (pst *RedisPresencePersistor) ClearPresence(ctx context.Context, userID uuid.UUID, connID uuid.UUID, username string, guest bool) (presence.PresenceChannelsDiff, error) {
	now := time.Now()
	expiration := now.Add(time.Minute * 2)
...
func (pst *RedisPresencePersistor) ListUsersInChannel(ctx context.Context, channel string) ([]presence.UserPresenceInfo, error) {
	...
	users := make([]presence.UserPresenceInfo, 0, len(userIDs))
	...
	for i, cmd := range cmds {
		...
		users = append(users, presence.UserPresenceInfo{
			ID:       userIDs[i],
