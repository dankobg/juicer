...
-- Create "game_result_status" table
...
-- Create "game_time_category" table
...
CREATE TABLE "game" (
  ...
  CONSTRAINT "game_check" CHECK (((white_id IS NOT NULL) AND (black_id IS NOT NULL) AND (guest_white_id IS NULL) AND (guest_black_id IS NULL)) OR ((guest_white_id IS NOT NULL) AND (guest_black_id IS NOT NULL) AND (white_id IS NULL) AND (black_id IS NULL)))
);
-- Create "game_move" table
...
CREATE TABLE "rating" (
  ...
  "updated_at" timestamptz NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY ("id"),
  CONSTRAINT "rating_user_id_game_time_category_id_key" UNIQUE ("user_id", "game_time_category_id"),
