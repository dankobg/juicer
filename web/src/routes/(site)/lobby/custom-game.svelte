<script lang="ts">
	...
	let {
		...
		gameVariants: components['schemas']['GameVariant'][];
		gameTimeKinds: components['schemas']['GameTimeKind'][];
	} = $props();
				...
				<Form.Control>
					{#snippet children({ props })}
						...
						<Select.Root type="single" bind:value={$form.variant} name={props.name}>
							...
							<Select.Content>
								{#each gameVariants as variant (variant.id)}
				...
				<Form.Control>
					{#snippet children({ props })}
						...
						<Select.Root type="single" bind:value={$form.timeMode} name={props.name}>
							...
							<Select.Content>
								{#each gameTimeKinds as timeKind (timeKind.id)}
